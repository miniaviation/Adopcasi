...
-- ── GUI ───────────────────────────────────────────────────────────────────────
local function showGui(partner, items, ItemDB, apiSuccess, apiError)
    ...
    local BTN_H    = 38
    local W        = 380
    local totalH   = PAD + TITLE_H + PARTNER_H + DIV_H + (#lines * ROW_H) + DIV_H + STATUS_H + DIV_H + BTN_H + PAD
    ...
    -- API status row
    local statusText = apiSuccess and "✔ Saved to database" or ("✘ DB error: " .. tostring(apiError))
    local statusColor = apiSuccess and Color3.fromRGB(80, 220, 80) or Color3.fromRGB(255, 80, 80)
    lbl(statusText, 13, statusColor, STATUS_H, false, 101)
...
local function sendToApi(partner, items, ItemDB)
    ...
    if ok and response and response.StatusCode == 200 then
        ...
        local errMsg = "no response"
        if response then
            errMsg = response.Body or tostring(response.StatusCode)
        end
        return false, errMsg
    end
end
    ...
    ClientData.register_callback_plus_existing("trade", function(_, newState)
        if newState == nil and lastTradeState ~= nil then
            if lastTradeState.current_stage == "confirmation" then
                ...
                -- Send to API then show GUI with result
                task.spawn(function()
                    local success, errMsg = sendToApi(partner, items, safeItemDB)
                    showGui(partner, items, safeItemDB, success, errMsg)
                end)
            end
        end
